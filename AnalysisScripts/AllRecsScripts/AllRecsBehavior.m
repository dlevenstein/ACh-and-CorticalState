...
subplot(4,4,12+gi)
plot(pupilphaseWh.(genotypes{gg}).Ybins,nanmean(abs(couplingbyamp.(genotypes{gg}).fracWh),3))
hold on
plot(pupilphaseWh.(genotypes{gg}).Ybins,nanmean(abs(couplingbyamp.(genotypes{gg}).meanDur),3))
plot(pupilphaseWh.(genotypes{gg}).Ybins,nanmean(abs(couplingbyamp.(genotypes{gg}).WhRate),3))
...
legend('frac','dur','rate','location','northwest')
