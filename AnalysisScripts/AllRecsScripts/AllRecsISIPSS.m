...
%%
ISIPSS = bz_CollapseStruct(ISIPSSAll);

%%
animalname = extractBefore(ISIPSS.name, '_');
genotype = extractBetween(ISIPSS.name,'_','.');
...
%%
for gg = 1:2
    ConditionalRate.(genotypes{gg}) = bz_CollapseStruct(ISIPSS.ConditionalRate(genotypeidx==gg),3,'mean',true);
%     
...
for sll = 1:length(LAYERS)
    ...
    if pll==1
        ylabel({['Cells, ',(LAYERS{sll})],'Pop Rate'})
    end
    
    end
end
NiceSave('PopRateandPSS',figfolder,genotypes{gg})
end
...
NiceSave('PopRateandPSS',figfolder,[])
...
NiceSave('LayerLFPISIMod',figfolder,(genotypes{gg}),'figtype','pdf')
...
NiceSave('LFPISIMod',figfolder,[],'figtype','pdf')
...
NiceSave('DepthPSSOscBehCorr',figfolder,groupnames{ff},'figtype','pdf')
...
NiceSave('DepthPSSandPup',figfolder,groupnames{ff},'figtype','pdf')
end
...
NiceSave(['LayerPSSbyPhase_',(ONOFF{oo})],figfolder,groupnames{ff},'figtype','pdf')
end
end
...
NiceSave('DepthPSSandWhisk',figfolder,groupnames{ff},'figtype','pdf')
end
%%
for ff = 1:2
...
NiceSave('PSSDepthComponents',figfolder,groupnames{ff},'figtype','pdf')
end
...
NiceSave(['PSSDistPupCycle_',WHNWH{ww}],figfolder,groupnames{ff},'figtype','pdf')
...
NiceSave(['PSSDistPupSize_',WHNWH{ww}],figfolder,groupnames{ff},'figtype','pdf')
...
NiceSave('PSSDistEMG',figfolder,groupnames{ff},'figtype','pdf')
