...
subplot(3,3,1:2)
    ...
    legend('WT: pup','WT: dpdt','EMX: pup','EMX: dpdt','location','eastoutside')
...
for gg = 1:length(genotypes)
    ...
    for gg = 1:length(genotypes)
        ...
        for rr = 1:length(recs)
            hold on
            plot(PSShist.bins,PSShist.ALL.hist(recs(rr),:),genotypecolors{gg})            
        end
        plot(PSShist.bins,PSShist.(genotypes{gg}),genotypecolors{gg},'linewidth',2)
    end
    axis tight
    xlabel('PSS');ylabel('# Time Bins')
    
 NiceSave('PSSandpupcycle',analysisfolder,'_')
%%
figure
for gg = 1:length(genotypes)
    recs = find(genotypeidx==gg);
