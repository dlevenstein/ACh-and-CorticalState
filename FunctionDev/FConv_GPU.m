function [ convolution_result_fft ] = FConv_GPU( kernel, signal, varargin )
...
nChans = size(fft_signal,2);
fft_kernel = repmat(fft_kernel,[1 nChans]);
convolution_result_fft = ifft((fft_kernel.*fft_signal),n_convolution);

% cut off edges
convolution_result_fft = gather(convolution_result_fft(half_of_kernel_size+1:end-half_of_kernel_size,:));
